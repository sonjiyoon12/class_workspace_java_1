...
public class MyBookStore {
    ...
    // 저장 하기
    // add(int n1, int n2)와 유사함
    public static void save(Scanner scanner, Book[] books) {
        System.out.println("---- save() ----");
        // 사용자에 값을 받아서 - 데이터를 모아서 북 개체를 생성하고 배열 공간에 넣아야 한다.
        System.out.println("책 제목을 입력하세요");
        String title = scanner.nextLine();
        System.out.println("책 저자를 입력하세요");
        String author = scanner.nextLine();
        Book book = new Book(title, author);
        // 5, 6, 7, 8 <-- 덮어쓰기되면 안되므로 어디까지 썼는지 알필요있음.
        // books[6] = new Book(title, author);
        // books[7] = new Book(title, author);
    ...
    public static void readBytitle(Scanner scanner, Book[] books) {
        ...
        // 찾는 기능을 만들어 줘야 한다.
        // [] * 100 다 확인 해봐야 합니다.
        // ["홍길동전"][] <- 100개중 몇 번째있는지 모름
        for (int i = 0; i < books.length; i++) {
            // 치환하면 "홍길동" 문자열이 됨.
            // "홍길동".equals()
            // 0 ~ 5,
            // 6번째 인덱스에 값은 null 가리키고 있다. - 오류 발생
            // 방어적 코드 작성해야 한다.
            if (books[i] != null) {
                // title.trim() <-- 공백 제거
    ...
    // 전체 삭제 하기  - 배열안에 담겨 있는 Book() 정보 null 처리
    // 전체 삭제 중
    // books = null; <- 메모리공간자체가 없어짐. 메모리공간 즉 배열을 다시 만등러줘야함.
    // 1. 배열 주소가지고와서 반복문 100 = null
