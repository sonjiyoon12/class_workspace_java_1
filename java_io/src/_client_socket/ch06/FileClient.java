...
public class FileClient {
    ...
    private void uploadFile() throws IOException {
        ...
        // 파일 이름이 "test.txt" 이라면 8칸 차지하고 나머지 92칸 비어 있음
        ...
        // 파일 내용을 바이트로 변환해서 보내 주어야 한다.
        try (FileInputStream fis = new FileInputStream(file)) {
            byte[] buffer = new byte[4096]; //4KB
            int bytesRead;
            while ((bytesRead = fis.read(buffer)) != -1){
                out.write(buffer,0,bytesRead); // 0번째 부터 읽은 만큼
            }
            out.flush(); // 다 보냈음
            ...
            String response = new String(responseBuffer, 0, responseLength); // 문자열로 변환
    ...
    public static void main(String[] args) {
        FileClient fileClient = new FileClient("손지윤");
